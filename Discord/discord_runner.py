...
time.sleep(3)
ps_output = [x for x in os.popen('ps -ef | grep \&\&\ python\ discord_runner.py').read().splitlines() if 'grep' not in x]
...
if len(ps_output) == 1:
	...
	os.system("kill $(ps aux | grep '[p]ython discord_runner.py' | awk '{print $2}')")
