...
def get_messages(sub_config, last_update_time):
	messages = []
	found_all = False
	start = True
	while not found_all:
		if start:
			iter_messages = send_request(GET, sub_config.discord_config.baseUrl+"?limit=100", sub_config.discord_config.headers).json()
		else:
			iter_messages = send_request(GET, sub_config.discord_config.baseUrl+"?after="+messages[-1]['id']+"&limit=100", sub_config.discord_config.headers).json()
		for message in iter_messages:
			print(message['timestamp'])
			ts = datetime.datetime.strptime(message['timestamp'], '%Y-%m-%dT%H:%M:%S.%f%z').timestamp()
			print(ts)
...
def main(sub_config):
	messages = get_messages(sub_config, timestamp)
...
if __name__ == '__main__':
	...
	for subname in subnames:
		...
		sub_config = Config(subname)
		if not sub_config.discord_config or sub_config.disabled:
			continue
		print("=== " + subname + " ===")
		main(sub_config)
